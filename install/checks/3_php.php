...
final class PHPCheck extends Check {
	...
	public function performCheck() {
		if (version_compare(PHP_VERSION, '5.0.0', '<')) {
			$this->message = '<p>You use PHP Version '.PHP_VERSION.'. This is definitly too old. You must update at least to PHP 5.3.0.</p>';
			$this->result = CheckResult::FAIL;
        } elseif(version_compare(PHP_VERSION, '5.3.0', '<')) {
            $this->message = '<p>Your PHP Version ('.PHP_VERSION.') is too old. You must update at least to PHP 5.3.0.</p>';
			$this->result = CheckResult::FAIL;
